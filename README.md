<div align="center">
  <!-- Bannière d'accueil dynamique et professionnelle -->
  <img src="https://capsule-render.vercel.app/api?type=waving&color=0:0A66C2,100:00B4D8&height=200&section=header&text=%F0%9F%90%B1%E2%80%8D%F0%9F%90%B2%20Bienvenue%20!&fontSize=38&fontColor=ffffff&animation=fadeIn&fontAlignY=35" width="100%"/>

  <h1> Moi c'est Nicole Manuella Afane Mvembe </h1>
  <p><i>Conception de logiciels performants & architecture de pipelines de données</i></p>

  <!-- Animation SVG de présentation -->
  <a href="https://git.io/typing-svg">
    <img src="https://readme-typing-svg.demolab.com?font=Fira+Code&weight=600&size=20&pause=1000&color=0A66C2&center=true&vCenter=true&width=650&lines=L3+Informatique+%40+Universit%C3%A9+de+Caen;Data+Engineering+%26+Intelligence+Artificielle;Stage+3+Mois+(2027)+%2B+Alternance+Master+(2027-2028)" alt="Typing SVG" />
  </a>
</div>

<br/>

---

### 🎯 Opportunités recherchées (2027)

* 💼 **Stage de 3 mois :** À partir du printemps / été **2027** *(Data / Génie Logiciel)*
* 🎓 **Alternance (Master 1 & 2) :** Année universitaire **2027-2028** *(Data Science, IA & Data Engineering)*
* 📍 **Localisation :** Caen & toute la France (Mobilité / Hybride / Télétravail)

---

### 👤 À propos & Langues

* 🎓 **Formation :** Licence 3 Informatique à l'Université de Caen Normandie
* 🎬 **Fun fact :** Je résous des bugs d'algorithmes plus vite que je ne choisis un film sur Netflix 💡
* 🗣️ **Langues parlées :**
  * 🇫🇷 **Français :** Langue maternelle
  * 🇬🇧 **Anglais :** Courant *(B2/C1 — Capable de travailler et documenter du code en anglais)*

<br/>

<div align="center">
  <b>Connect with me:</b><br/><br/>
  <a href="https://www.linkedin.com/in/nicole-manuella-afane-mvembe">
    <img src="https://img.shields.io/badge/LinkedIn-Nicole%20Manuella%20Afane%20Mvembe-0A66C2?style=for-the-badge&logo=linkedin&logoColor=white"/>
  </a>
  <a href="mailto:afanemvembe@gmail.com">
    <img src="https://img.shields.io/badge/Email-afanemvembe%40gmail.com-EA4335?style=for-the-badge&logo=gmail&logoColor=white"/>
  </a>
  <a href="https://github.com/afanemvembe-alt">
    <img src="https://img.shields.io/badge/GitHub-afanemvembe--alt-181717?style=for-the-badge&logo=github&logoColor=white"/>
  </a>
</div>

---

### 🛠️ Languages & Tools

<p align="left">
  <img src="https://skillicons.dev/icons?i=python,java,c,php,js,html,css,postgres,mongodb,graphql,mysql,git,github,docker,linux,flask,bash&perline=7" />
</p>

---

### 🏆 GitHub Trophies

<p align="center">
  <img src="https://github-profile-trophies.vercel.app/?username=afanemvembe-alt&theme=flat&no-frame=true&column=4" alt="GitHub Trophies" />
</p>

---

### 🚀 Projets Majeurs

* **📊 Pipeline ETL & Dashboard Data** *(En cours)* — `Python` `PostgreSQL` `Docker` `Streamlit`  
  *Collecte automatisée d'API, nettoyage de données, modélisation SQL et visualisation interactive.*
* **🧩 Jeu du Taquin (MVC)** — `Java` `JavaFX` `Swing`  
  *Architecture MVC hybride (Console & GUI) avec algorithme de mélange garantissant la résolvabilité.*
* **🌿 Interprète de L-Systems** — `Java` `Design Patterns`  
  *Génération fractale 2D/3D optimisée en mémoire à l'aide des patterns Composite, Memento & Flyweight.*
* **🍳 Web App Recettes Full-Stack** — `PHP` `MySQL` `JavaScript` `HTML/CSS`  
  *Application web dynamique orientée MVC avec gestion de la base de données relationnelle et DOM JS.*
